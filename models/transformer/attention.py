...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def forward(
        ...
        k = (
            torch.cat([self.fc_k(keys), m_k], 1)
            .view(b_s, nk + self.m, self.h, self.d_k)
            .permute(0, 2, 3, 1)
        )  # (b_s, h, d_k, nk)
        v = (
            torch.cat([self.fc_v(values), m_v], 1)
            .view(b_s, nk + self.m, self.h, self.d_v)
            .permute(0, 2, 1, 3)
        )  # (b_s, h, nk, d_v)
...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        if self.identity_map_reordering:
            ...
            out = self.attention(
                q_norm, k_norm, v_norm, attention_mask, attention_weights
            )
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(
                queries, keys, values, attention_mask, attention_weights
            )
